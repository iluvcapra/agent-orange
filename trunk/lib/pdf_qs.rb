...
def pdf_for_session(s_obj , paper, strips_per_page)
  ...
  time_font_size = s_obj.time_font_size
  finish_time_font_size = s_obj.finish_time_font_size
  ...
  { "Width between left-right margins" => p.margin_width, 
    "Width per strip" => strip_width,
    "Grid top position" => grid_top,
    "Grid bottom position" => grid_bottom,
    "Cue area top position" => cue_top,
    "Strip header height" => strip_header_height,
    "Channel header height" => channel_header_height
  }.each do |k,v|
    #$stderr.print "%40s : %f\n" % [k , v]
  end
